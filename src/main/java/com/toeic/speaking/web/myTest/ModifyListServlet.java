...
public class ModifyListServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if(loginedUser == null) {
			resp.sendRedirect("../login?fail=deny&job=" + URLEncoder.encode("시험접수확인 페이지는 로그인 이후 이용가능합니다.", "utf-8"));
			return;
		}
		...
		List<TestHistoryDto> testHistoryDtos = testHistoryDao.getAllTestHistoryByUserId(loginedUserId);
		...
		req.getRequestDispatcher("/WEB-INF/views/mytest/modifyList.jsp").forward(req, resp);
