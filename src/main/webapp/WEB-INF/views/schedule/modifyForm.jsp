						...
						<div>
							<select class="js-select2" name="test" id="test" onchange="disPlayOldDataForm(this.value);">
								<c:forEach var="dtos" items="${scloDto }">
									...
									<fmt:formatDate value="${dtos.testDate }" pattern="yyyy-MM-dd"/>: ${dtos.testName } - ${dtos.centerName }</option>
									</c:if>
								</c:forEach>
							</select>
...
<script type="text/javascript">
	...
	function disPlayOldDataForm(testNo){
		...
		if(xhr.readyState == 4 && xhr.status == 200){
			...
			var THMS = moment(new Date(data.testTime));
...
rows +=         "<div>"
rows +=            "<input type='text' class='wrap-input100 input100-select bg1' id='testDateOld' name='testDateOld' value='"+THMS.format('YYYY-MM-DD a hh:mm')+"' disabled/>"
rows +=         "</div>"
...
rows +=			"<div>"
rows +=				"<input type='number' class='wrap-input100 input100-select bg1' id='registerCount' name='quotaOld' value='"+data.quota+"' disabled />"
rows +=			"</div>"
